...
def get_class_share(df: pd.DataFrame, limit: int) -> dict:
    ...
    label_share = {label: min(count, round(total_rows / len(label_counts))) for label, count in label_counts.items()}
...
def limit_labeles_of_class(df: pd.DataFrame, limit: int) -> pd.DataFrame:
    ...
    sampled_dfs = [df[df['label'] == label].sample(n=label_share[label], random_state=1) for label in label_share]
...
def limit_classes_of_split(df: pd.DataFrame, limit: int) -> pd.DataFrame:
    ...
    for class_share in class_shares:
        ...
        labels_limited_of_class = limit_labeles_of_class(df_class, class_shares[class_share])
